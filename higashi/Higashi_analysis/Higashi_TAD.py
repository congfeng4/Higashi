    ...
    def fit_transform(self, sc_score, sc_boundaries, bulk_tad_d):
        ...
        while True:
            ...
            for cell in bar:
                ...
                cell, sc_assignment_cell, search_range_cell = self.assign(cell, len(self.shared_boundaries),
                                                                          self.shared_boundaries, sc_score[cell],
                                                                          cum_score[
                                                                              cell], c_bound)
            ...
            bar = trange(len(self.shared_boundaries),
                         desc="%s Epoch %d M-step: - " % (self.print_identifier, epoch_count))
            for j in range(len(shared_boundaries)):
                start, end = search_range[j]
                if end > start:
                    p_list.append(
                        pool.submit(self.update, n_cell, sc_boundaries, sc_assignment, sc_score, cum_score, j, start,
                                    end))
            ...
            bar.set_description("%s Epoch: %d - update_ratio = %f " % (self.print_identifier, epoch_count,
                                                                       change / len(self.shared_boundaries)),
                                refresh=False)
            self.shared_boundaries = list(np.unique(self.shared_boundaries))
            self.shared_boundaries.sort()
        ...
        new_sc_assignment = np.array(
            [[boundaries2assignment[b] for b in calib_b] for calib_b in calibrated_sc_boundaries])
