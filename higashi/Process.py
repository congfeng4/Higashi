                ...
                with open(os.path.join(data_dir, "data.txt"), 'r') as csv_file:
                    ...
                    for chunk in reader:
                        if len(chunk['cell_id'].unique()) == 1:
                            ...
                            p_list.append(
                                pool.submit(data2triplets, config, cell_tab.copy(deep=True), chrom_start_end, False))
                            cell_tab = [head]
...
def create_matrix_one_chrom(config, c, size, cell_size, temp, temp_weight, chrom_start_end, cell_num, total_part_num=1,
                            part_num=0, per_cell_read=1000):
    ...
    with warnings.catch_warnings():
        ...
        if total_part_num == 1:
            ...
            np.save(os.path.join(temp_dir, "temp", "sparse_gcn_%s.npy" % chrom_list[c]), np.array(sparse_list_for_gcn),
                    allow_pickle=True)
            np.save(os.path.join(temp_dir, "temp", "cell_adj_%s.npy" % chrom_list[c]), cell_adj,
                    allow_pickle=True)
        else:
            np.save(os.path.join(temp_dir, "temp", "%s_sparse_adj_part_%d.npy" % (chrom_list[c], part_num)),
                    sparse_list)
            np.save(os.path.join(temp_dir, "temp", "sparse_gcn_%s_part_%d.npy" % (chrom_list[c], part_num)),
                    np.array(sparse_list_for_gcn),
                    allow_pickle=True)
            np.save(os.path.join(temp_dir, "temp", "cell_adj_%s_part_%d.npy" % (chrom_list[c], part_num)), cell_adj,
                    allow_pickle=True)
        return np.array(read_count).reshape((-1)), c, np.array(a), b, total_part_num, part_num, cell_id, np.asarray(
            qc_list)
...
def create_matrix(config, disable_mpl=False):
    ...
    with h5py.File(os.path.join(temp_dir, "node_feats.hdf5"), "a") as save_file:
        ...
        for c in range(len(chrom_list)):
            ...
            if save_mem:
                ...
                for part in range(split_num):
                    ...
                    if not disable_mpl:
                        p_list.append(
                            pool.submit(create_matrix_one_chrom, config, c, size, cell_size, temp[mask],
                                        temp_weight[mask], chrom_start_end,
                ...
                if not disable_mpl:
                    p_list.append(
                        pool.submit(create_matrix_one_chrom, config, c, size, cell_size, temp, temp_weight,
                                    chrom_start_end, cell_num, 1, 0, per_cell_read))
                else:
                    p_list.append(
                        [config, c, size, cell_size, temp, temp_weight, chrom_start_end, cell_num, 1, 0, per_cell_read])
                cell_feats[c] = [[]]
                qc_list[c] = [[]]
            ...
            def pseudo_bulk():
                bin_adj = csr_matrix((temp_weight_mask, (
                    temp_mask[:, 2] - chrom_start_end[c, 0], temp_mask[:, 3] - chrom_start_end[c, 0])),
                                     shape=(size, size), dtype='float32')
                bin_adj = np.array(bin_adj.todense())
                bin_adj = bin_adj + bin_adj.T + np.diag(
                    np.sum(bin_adj, axis=-1) == 0)  # - np.diag(np.diagonal(bin_adj))
            ...
            if total_part_num == 1:
                ...
                for pt in range(total_part_num):
                    ...
                    cell_adj_all.append(np.load(
                        ...
                        allow_pickle=True))
                    origin_sparse_list.append(
                        np.load(os.path.join(temp_dir, "temp", "%s_sparse_adj_part_%d.npy" % (chrom_list[c], pt)),
                                allow_pickle=True))
        ...
        for c in range(len(chrom_list)):
            ...
            create_or_overwrite(save_file, "train_weight_%s" % chrom_list[c],
                                data=weight[mask][train_index].astype('float32'))
            ...
            create_or_overwrite(save_file, "test_weight_%s" % chrom_list[c],
                                data=weight[mask][test_index].astype('float32'))
