    ...
    def ps1(batch_edge_big, batch_y_big, batch_edge_weight_big, batch_chrom_big, batch_to_neighs_big, chroms_in_batch):
        ...
        if train_p_list is not None:
            ...
            while len(train_p_list) < batch_num:
                ...
                train_p_list.append(
                    train_pool.submit(one_thread_generate_neg, edges_part, edges_chrom, edge_weight_part, collect_num,
                                      True,
                                      chroms_in_batch))
...
def train(model, loss, training_data_generator, validation_data_generator, optimizer, epochs, load_first,
    ...
    if cpu_num > 1:
        train_pool = ThreadPoolExecutor(max_workers=cpu_num)
        eval_pool = ThreadPoolExecutor(max_workers=cpu_num)
        train_p_list = []
    else:
        print("no parallel")
