...
from bokeh.models import ColorBar, LinearColorMapper, BasicTicker, BoxSelectTool, LassoSelectTool, LabelSet, HoverTool, \
...
def plot_heatmap_RdBu_tad(matrix, normalize=True, cbar=False, cmap=None, force_vmin=None):
    ...
    # ax.imshow(matrix, cmap=cmap,interpolation='none', vmin=vmin, vmax=vmax)
        ...
        vmin, vmax = np.quantile(matrix[(matrix > 1e-8)], cutoff), np.quantile(matrix[(matrix > 1e-8)],
                                                                               1 - cutoff) + 1e-8
...
def async_heatmap22(selected, id):
    try:
        ...
        with h5py.File(os.path.join(temp_dir, chrom_selector.value + "_" + embedding_name + "_nbr_0_impute.hdf5"),
                       "r") as f:
            coordinates = f['coordinates']
            xs, ys = coordinates[:, 0], coordinates[:, 1]
            p = 0
...
def async_heatmap31(selected, id):
    try:
        ...
        with h5py.File(os.path.join(temp_dir, "%s_%s_nbr_%d_impute.hdf5" % (chrom_selector.value, embedding_name,
                                                                            neighbor_num - 1)),
                       "r") as f:
            coordinates = f['coordinates']
            xs, ys = coordinates[:, 0], coordinates[:, 1]
...
def float_color_update(s, vmin=None, vmax=None):
    ...
    continuous_h_all.data_source.data = dict(x=(hedges[:-1] + hedges[1:]) / 2, top=hhist,
                                             fill_color=[color1] * len(hhist))
    ...
    color_s = np.clip(s, a_min=np.quantile(s, 0.05) if vmin is None else vmin,
                      a_max=np.quantile(s, 0.95) if vmax is None else vmax)
    # print (color_s)
    ...
    bar.update(color_mapper=LinearColorMapper(palette=pal, low=np.quantile(s, 0.05) if vmin is None else vmin,
                                              high=np.quantile(s, 0.95) if vmax is None else vmax))
    bar.visible = True
    return
...
def str_color_update(s, new):
    ...
    if 'vis_palette' not in config:
        if len(l) <= 10:
            ...
            Category20_20_temp = list(Category20_20_temp) + list(np.array(Spectral11)) + list(
                Category20_20_temp) + list(np.array(Spectral11))
...
dim_reduction_selector = Select(title='Vis method', value="PCA",
                                options=["PCA", "UMAP", "TSNE", "MDS-euclidean", "MDS-cosine"], width=150)
...
heatmap12 = figure(toolbar_location="above", tools="pan, wheel_zoom, reset, save", plot_width=300, plot_height=300,
                   x_range=heatmap11.x_range, y_range=heatmap11.y_range,
                   min_border=5, output_backend="webgl", title='conv-rwr', active_scroll="wheel_zoom")

heatmap22 = figure(toolbar_location="above", tools="pan, wheel_zoom, reset, save", plot_width=300, plot_height=300,
                   x_range=heatmap11.x_range, y_range=heatmap11.y_range,
                   min_border=5, output_backend="webgl", title='Higashi(0)', active_scroll="wheel_zoom")

heatmap31 = figure(toolbar_location="above", tools="pan, wheel_zoom, reset, save", plot_width=300, plot_height=300,
                   x_range=heatmap11.x_range, y_range=heatmap11.y_range,
...
chrom_selector = Select(title='chromosome selector', value=config['chrom_list'][0], options=config['chrom_list'],
                        width=150)
...
plot_distance_selector = Slider(title='Heatmap distance', value=origin_sparse[0].shape[-1], start=1,
                                end=origin_sparse[0].shape[-1], step=1, width=150, value_throttled=2000)
local_selection_slider = Slider(title='Local selection number', value=1, start=1, end=origin_sparse[0].shape[-1],
                                step=1, width=150, value_throttled=2000)
...
r = embed_vis.scatter(x="x", y="y", size=size_selector.value, fill_color="color", line_color=None,
                      legend_field="legend_info",
                      alpha=0.9, source=source, nonselection_fill_alpha=0.9, selection_fill_color="color",
...
categorical_info = figure(toolbar_location=None, x_range=bar_info, plot_width=300, plot_height=300, min_border=5,
                          output_backend="webgl", title='None bar plot')
...
continuous_info = figure(toolbar_location=None, plot_width=300, plot_height=300, min_border=5, output_backend="webgl",
                         title='None histogram')
...
matrix_start_slider_x = Slider(title="Heatmap start: x", value=0, start=0, end=origin_sparse[0].shape[-1], step=1,
                               value_throttled=2000, width=150)
matrix_end_slider_x = Slider(title="Heatmap end: x", value=origin_sparse[0].shape[-1], start=0,
                             end=origin_sparse[0].shape[-1], step=1, value_throttled=2000, width=150)

matrix_start_slider_y = Slider(title="Heatmap start: y", value=0, start=0, end=origin_sparse[0].shape[-1], step=1,
                               value_throttled=2000, width=150)
matrix_end_slider_y = Slider(title="Heatmap end: y", value=origin_sparse[0].shape[-1], start=0,
                             end=origin_sparse[0].shape[-1], step=1, value_throttled=2000, width=150)
vmin_vmax_slider = Slider(title='Vmin/Vmax(% of range)', value=0.95, start=0.0, end=1.0, step=0.01,
                          value_throttled=2000, width=150)
...
def change_theme(button):
    ...
    if blackorwhite == "white":
        ...
        blackorwhite = "black"
        for selector in [data_selector, chrom_selector, dim_reduction_selector, color_selector, xy_selector,
                         heatmap_color_selector]:
