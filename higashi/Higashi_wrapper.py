...
def mp_impute(config_path, path, name, mode, cell_start, cell_end, sparse_path, weighted_info=None, gpu_id=None):
    ...
    cmd = ["python", impute_file_path, config_path, path, name, mode, str(int(cell_start)), str(int(cell_end)),
           sparse_path]
...
class Higashi():
    ...
    def fetch_info_from_config(self):
        ...
        if "phasing_epoch" in config:
            self.phasing_epoch = config['phasing_epoch']
        else:
            self.phasing_epoch = 45
    ...
    def generate_attributes(self):
        ...
        with h5py.File(os.path.join(self.temp_dir, "node_feats.hdf5"), "r") as save_file:
            ...
            if self.pre_cell_embed:
                ...
                targets.append(StandardScaler().fit_transform(cell_node_feats.reshape((-1, 1))).reshape(
                    (len(cell_node_feats), -1)))
                embeddings.append(StandardScaler().fit_transform(cell_node_feats.reshape((-1, 1))).reshape(
                    (len(cell_node_feats), -1)))
    ...
    def forward_batch_hyperedge(self, batch_data, batch_weight, batch_chrom, batch_to_neighs, y,
        ...
        pred, pred_var, pred_proba, (pred_p, pred_m) = model(x, (batch_chrom, batch_to_neighs),
        ...
        if steps == 4:
            def postprocess(prediction):
                ...
                prediction = torch.clamp(prediction, min=1e-8, max=1e8)
                return prediction

            pred_p = postprocess(pred_p)
            pred_m = postprocess(pred_m)

            phase_loss = -log_zinb_positive(w.float(), (pred_p + pred_m).float(), pred_var.float(), pred_proba.float())
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.num_list)):
                ...
                if i == 0:
                    try:
                        old_static = np.load(
                            os.path.join(self.embed_dir, "%s_%d_origin.npy" % (self.embedding_name, i)))
    ...
    def get_cell_neighbor_be(self, start=1):
        ...
        return np.array(cell_neighbor_list_local, dtype='object'), np.array(cell_neighbor_weight_list_local,
                                                                            dtype='object')
    ...
    def train_for_phasing(self, max_epochs=None):
        print('Training for phasing')
        global steps, pair_ratio
        # 加载 stage1 模型
        ...
        self.train(
            ...
            validation_data_generator=self.validation_data_generator,
            optimizer=[optimizer],
            epochs=max_epochs or self.phasing_epoch,
            load_first=False,
            save_name="_phasing",
            save_embed=False
        )
    ...
    def phase(self):
        ...
        结果保存为：
            chr{id}_Cp_sparse_adj.npy   # (n_cells,)  csr_matrix  float32
            chr{id}_Cm_sparse_adj.npy   # (n_cells,)  csr_matrix  float32
            chr{id}_C_sparse_adj.npy    # (n_cells,)  csr_matrix  float32  (原始)
        不再生成 hdf5
        """
        from scipy.sparse import csr_matrix
        ...
        # 1. 加载 phasing 模型
        self.higashi_model = torch.load(self.save_path + "_phasing_model",
        ...
        for c_idx, chrom in enumerate(self.chrom_list):
            ...
            # 预分配 object 数组，存放每个 cell 的 csr_matrix
            cp_arr = np.empty(n_cells, dtype=object)   # 父本
            cm_arr = np.empty(n_cells, dtype=object)   # 母本

            for cell in range(n_cells):
                raw_csr = self.fetch_map(chrom, cell)[0].astype(np.float32)
                rows, cols = raw_csr.nonzero()
                counts = raw_csr.data

                if len(counts) == 0:          # 空矩阵直接存
                    empty = csr_matrix((n_bins, n_bins), dtype=np.float32)
                    cp_arr[cell] = empty
                    cm_arr[cell] = empty
                    continue
                ...
                x = np.column_stack([np.full(len(rows), cell + 1),
                                     rows + s + 1,
                                     cols + s + 1])          # (n_edges, 3)  int64

                # 染色体编号：每条边对应同一个 c_idx
                x_chrom = np.full(len(x), c_idx, dtype=np.int64)   # <- numpy 数组

                # predict 期望 numpy，内部自行 .to(device)
                output, output_p, output_m = self.higashi_model.predict(
                    x, x_chrom, activation=F.softplus)

                output, output_p, output_m = self.higashi_model.predict(
                    x, x_chrom, activation=F.softplus)

                # 转回 host & 强制 1-D
                cp_data = output_p.flatten()
                cm_data = output_m.flatten()

                cp_arr[cell] = csr_matrix((cp_data, (rows, cols)), shape=(n_bins, n_bins))
                cm_arr[cell] = csr_matrix((cm_data, (rows, cols)), shape=(n_bins, n_bins))

            # 3. 保存为 npy（1 文件 / 染色体 / 类型）
            base_path = os.path.join(self.temp_dir, f"phase/{chrom}")
            np.save(base_path + "_Cp_sparse_adj.npy", cp_arr)
            np.save(base_path + "_Cm_sparse_adj.npy", cm_arr)

        print("Phased Hi-C saved to chr<id>_{Cp,Cm}_sparse_adj.npy")
    ...
    def impute_no_nbr(self):
        ...
        if self.non_para_impute:
            ...
            impute_process(self.config_path, self.higashi_model, "%s_nbr_%d_impute" % (self.embedding_name, 0),
                           self.mode, 0,
                           self.num[0],
    ...
    def impute_with_nbr(self):
        ...
        if self.non_para_impute:
            cell_id_all = [np.arange(self.num[0])]
            impute_process(self.config_path, self.higashi_model,
                           "%s_nbr_%d_impute" % (self.embedding_name, self.neighbor_num - 1),
                           self.mode, 0,
            ...
            linkhdf5("%s_nbr_%d_impute" % (self.embedding_name, self.neighbor_num - 1), cell_id_all, self.temp_dir,
                     self.impute_list,
                     None)
    ...
    def fetch_map(self, chrom, cell):
        ...
        size = e - s
        try:
            with h5py.File(os.path.join(self.temp_dir, "%s_%s_nbr_%d_impute.hdf5" % (chrom, self.embedding_name, 0)),
                           "r") as f:
        ...
        try:
            with h5py.File(os.path.join(self.temp_dir, "%s_%s_nbr_%d_impute.hdf5" % (chrom, self.embedding_name,
                                                                                     self.neighbor_num - 1)), "r") as f:
                coordinates = np.array(f['coordinates']).astype('int')
                p = np.array(f["cell_%d" % cell])
        ...
        if chrom not in self.ori_sparse_list:
            self.ori_sparse_list[chrom] = np.load(os.path.join(self.temp_dir, "raw", "%s_sparse_adj.npy" % chrom),
                                                  allow_pickle=True)
